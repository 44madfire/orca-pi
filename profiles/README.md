...
- Tokens resolve at runtime (`ORCA_PI_GITHUB_<IDENTITY>_TOKEN` + optional `..._EXPIRES_AT` ISO-8601); mint/refresh outside LLM context. Never place keys/tokens/secrets in prompts, task text, logs, or Linear.
- Same-account PATs are not distinct actors — install the reviewer App for a separate actor.
- CLI: `orca-pi github auth status --identity reviewer`, `orca-pi github review ...`, `orca-pi github check start|complete ...` (see root `README.md`). Helpers redact token-like values from output/errors.
